...
const CurriculumSection = ({ curriculum, trainingSessionId, jwtToken, refreshToken, hydrate }: CurriculumSectionProps) => 
{
    ...
    return (
        <div className="mt-8">
            <h2 className="font-serif text-3xl font-bold mb-4 text-center text-ink-400">{curriculum.session_title}</h2>
            <p className="text-lg text-slate-zen400 mb-6 text-center">Total Duration: {curriculum.duration}</p>

            {/* Navigation Bar */}
            <nav className="flex flex-wrap justify-center space-x-4 mb-6 bg-parchment-200 text-ink-400 p-2 rounded-lg sticky top-0 z-10 shadow-zen-sm">
            ...
            <section id="warm-up" className="mb-6">
                <div className="bg-orange-100 p-4 rounded-lg shadow-zen-sm">
                    <div className="flex items-center justify-between">
                        <h3 className="text-2xl font-semibold text-orange-800">Warm-Up</h3>
                    </div>
                    <p className="text-lg font-bold mt-2">{curriculum.warm_up.name}</p>
                    <p className="text-slate-zen400">{formatDescriptionWithNumberedList(curriculum.warm_up.description)}</p>
                    <p className="text-sm text-slate-zen400">Duration: {curriculum.warm_up.duration}</p>
                    <DrillTimer
            ...
            <section id="techniques" className="mb-6">
                <div className="bg-blue-100 p-4 rounded-lg shadow-zen-sm">
                    <h3 className="text-2xl font-semibold text-blue-800">Techniques</h3>
                    <div className="mt-2">
                        {curriculum.techniques.map((tech, index) => (
                            <div key={index} className="border-b border-blue-200 py-2">
                                <div className="flex items-center justify-between">
                                    <span className="w-full text-left text-lg font-bold text-blue-700 focus:outline-none">{tech.name}</span>
                                    <button type="button" onClick={() => toggleAccordion(`tech-${index}`)} className="p-3 text-blue-700 focus:outline-none">
                                ...
                                <div className={`mt-2 ${expandedSections[`tech-${index}`] ? 'block' : 'hidden'}`}>
                                    ...
                                    <p className="relative group">
                                        ...
                                        <span className="absolute hidden group-hover:block bg-parchment-50 text-ink-400 text-sm rounded p-2 -mt-10 w-64 shadow-zen-md z-20">
                                            {tech.tips}
                                        </span>
                                    </p>
            ...
            <section id="drills" className="mb-6">
                <div className="bg-green-100 p-4 rounded-lg shadow-zen-sm">
                    <h3 className="text-2xl font-semibold text-green-800">Drills</h3>
                    <div className="mt-2">
                        {curriculum.drills.map((drill, index) => (
                            <div key={index} className="border-b border-green-200 py-2">
                                <div className="flex items-center justify-between">
                                    <span className="w-full text-left text-lg font-bold text-green-700 focus:outline-none">{drill.name}</span>
                                    <button
                                        className="p-3 text-green-700 flex justify-between items-center focus:outline-none"
                                ...
                                <div className={`mt-2 ${expandedSections[`drill-${index}`] ? 'block' : 'hidden'}`}>
                                    ...
                                    <p className="text-sm text-slate-zen400">Duration: {drill.duration}</p>
            ...
            <section id="sparring" className="mb-6">
                <div className="bg-red-100 p-4 rounded-lg shadow-zen-sm">
                    <div className="flex items-center justify-between">
                        <h3 className="text-2xl font-semibold text-red-800">Sparring</h3>
                    </div>
                    ...
                    <p className="text-sm text-slate-zen400">Duration: {curriculum.sparring.duration}</p>
                    <DrillTimer
            ...
            <section id="cool-down" className="mb-6">
                <div className="bg-purple-100 p-4 rounded-lg shadow-zen-sm">
                    <div className="flex items-center justify-between">
                        <h3 className="text-2xl font-semibold text-purple-800">Cool-Down</h3>
                    </div>
                    <p className="text-lg font-bold mt-2">{curriculum.cool_down.name}</p>
                    <p className="text-slate-zen400">{curriculum.cool_down.description}</p>
                    <p className="text-sm text-slate-zen400">Duration: {curriculum.cool_down.duration}</p>
                    <DrillTimer
